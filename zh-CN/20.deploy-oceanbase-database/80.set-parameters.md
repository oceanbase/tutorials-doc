...
* 租户级参数指适用于租户级别的配置选项，它们是针对单个租户或多个租户的配置选项。用于对单个租户或多个租户进行特定的配置和优化。这些配置项通常包括存储引擎参数、SQL 执行策略、访问控制等方面的配置选项。租户级配置项通常可以在租户创建和管理时进行配置，可以随时根据需要进行修改。
...
* 通过 OBD 查看参数
  ...
  <!-- 这样查看的只是 OBD 配置文件中出现的几个配置项，并不是全部的配置项呀 -->
  ```shell
...
* 通过 SQL 命令行查看参数
  ...
  * 执行 show parameters 命令查看，示例如下：
    ...
    <!-- 需要明确下是 sys 租户就可以查询，还是 root@sys 才可以查询 -->
  * 查询 `GV$OB_PARAMETERS` 表，示例如下：
    ...
    <!-- - need_reboot:表示修改参数后是否需要重启才能生效。--这个是哪里出现的呀 -->
...
目前没有相关的表或者视图可以查看一个变量是否是只读的，需要在对应版本的以下源码包中获取。
<!-- 读不懂 -->
```shell
...
如果您需要在OCP查看变量，可以从官网[租户的参数](https://www.oceanbase.com/docs/common-ocp-1000000000584892)查看，因为OCP在租户的参数就是指变量。
<!-- OCP 在租户中的参数就是指变量，所有 OCP 中没有全局变量的概念么，并且 OCP 中也没有租户参数的概念？ -->
...
由于OCP在租户的参数就是变量，因此修改租户参数详见[修改租户的参数](https://www.oceanbase.com/docs/common-ocp-1000000000584890)。
