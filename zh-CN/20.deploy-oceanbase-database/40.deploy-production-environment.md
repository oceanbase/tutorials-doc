...
在本章第一节 [2.1 部署前准备](10.preparation-before-deployment.md) 中已经详细介绍了部署生产环境所需的部署环境要求，本节将以生产环境最低资源要求介绍如何规划 OceanBase 集群的部署。
...
* OCP 服务器资源要求是什么？
  ...
  以 OCP 管理的 OBServer 节点数量在 10 台以内为例，MetaDB 和 OCP 共用同台服务器时，这台物理机需要有 17 个 CPU 和 60 GB 内存。详细的资源要求可参见官网《OceanBase 云平台》文档 [部署 OceanBase 云平台/部署社区版 OceanBase 云平台/安装规划/主机规划](https://www.oceanbase.com/docs/common-ocp-1000000000368845)。
...
| OceanBase 数据库  | <ul><li>建议使用 ob-operator 推荐的 OceeanBase 数据库版本。</li><li>推荐部署 3 节点集群，且按物理机推荐资源要求设置。</li></ul> | <ul><li>ob-operator 2.1.2 对应的 OceanBase 数据库镜像版本为 4.2.1.3。</li><li> OceanBase 数据库资源 16C 32G，因此容器最低资源配置为 CPU：16，内存：36 GB（默认按 90% 分配给 OcaeanBase 数据库）。</li></ul> |
| K8S         | 推荐使用 calico 网络插件。                                                                              | 多个 OBServer 节点通过 nodeSelector 分配到不同的宿主机节点，以避免 Pod 故障重启情况下 IP 变化导致集群失活。                                                                     |
<!-- 表格中第二行的资源介绍和 2.1 冲突 -->
