...
<!-- 业务租户还是用户租户需要统一下 -->
强烈推荐创建并使用业务租户，禁止使用管理租户（sys 租户）做测试或业务使用，推荐参考本节内容选择合适的方法创建并使用业务租户。
...
1. 参照下图配置待租户的信息
   ...
   ![image.png](https://intranetproxy.alipay.com/skylark/lark/0/2024/png/35856333/1708395033617-f809a5b2-78a2-4d95-941f-992ab5c024b8.png#clientId=uc840ff5a-474c-4&from=paste&height=640&id=lNSJ3&originHeight=1280&originWidth=2784&originalType=binary&ratio=2&rotation=0&showTitle=false&size=550585&status=done&style=none&taskId=ua0e9e537-ba35-48fe-8377-57c69fe63b3&title=&width=1392)

2. 按需配置租户 IP 白名单和租户变量

   ![image.png](https://intranetproxy.alipay.com/skylark/lark/0/2024/png/35856333/1708344322842-a9c742d7-ce48-47d7-ab9b-b64db9bc0cc1.png#clientId=u2820323f-7c1c-4&from=paste&height=654&id=t8NSa&originHeight=1308&originWidth=2774&originalType=binary&ratio=2&rotation=0&showTitle=false&size=547522&status=done&style=none&taskId=u26253e80-4d30-4043-bce0-15b6e5336fb&title=&width=1387)

3. 查看 OCP 创建租户的任务

   如果想了解具体的执行过程，可以下载日志。

   ![image.png](https://intranetproxy.alipay.com/skylark/lark/0/2024/png/35856333/1708394918293-5c2fca8b-53ce-4209-9042-232827a88cc2.png#clientId=uc840ff5a-474c-4&from=paste&height=659&id=gXVmj&originHeight=1318&originWidth=2778&originalType=binary&ratio=2&rotation=0&showTitle=false&size=1276200&status=done&style=none&taskId=ubc66776a-a098-40d0-a204-0da9fe75457&title=&width=1389)
...
若待新建租户的集群是使用 OBD 部署的，可使用 OBD 命令来创建租户。

```bash
obd cluster tenant create ob422 -n test2 \
...
* 命令里的 `ob422` 是示例的集群名（`deploy name`），可执行 `obd cluster list` 命令，根据输出中 `Name` 列值进行替换。
...
```shell
[2024-02-19 19:09:04.385] [DEBUG] - cmd: ['ob422']
...
[2024-02-19 19:09:04.386] [DEBUG] - try to get exclusive lock /home/admin/.obd/lock/deploy_ob422
[2024-02-19 19:09:04.386] [DEBUG] - exclusive lock `/home/admin/.obd/lock/deploy_ob422`, count 1
...
[2024-02-19 19:09:38.645] [DEBUG] - exclusive lock /home/admin/.obd/lock/deploy_ob422 release, count 0
[2024-02-19 19:09:38.645] [DEBUG] - unlock /home/admin/.obd/lock/deploy_ob422
...
1. 创建资源单元规格
   ...
   create resource unit u1 min_cpu=2,max_cpu=2,memory_size='2g', log_disk_size='6g',max_iops=10000;
   
   create resource unit u2 min_cpu=2,max_cpu=2,memory_size='2g', log_disk_size='6g',max_iops=10000;
   ```

2. 使用 root 用户登录 OceanBase 数据库的 sys 租户，查看资源单元规格详情
   ...
   |           1002 | u1              | 2024-02-19 15:58:08.687997 | 2024-02-19 15:58:08.687997 |       2 |       2 |  2147483648 |    6442450944 |               10000 |               10000 |           0 |
   |           1003 | u2              | 2024-02-19 15:58:09.272046 | 2024-02-19 15:58:09.272046 |       2 |       2 |  2147483648 |    6442450944 |               10000 |               10000 |           0 |
   +----------------+-----------------+----------------------------+----------------------------+---------+---------+-------------+---------------+---------------------+---------------------+-------------+
   ```
...
3. 使用 root 用户登录 OceanBase 数据库的 sys 租户，查看集群剩余可用资源
   ...
   > **说明**
   >
   > 资源池成功创建后，集群的可用资源就减少了。
<!-- 这个说明是想要说 资源池创建后就会占用资源么，和前文 创建资源池时会实际创建 Unit，按照规格定义分配资源 是否重复？ -->
### 步骤三：创建租户

创建租户时通过指定 `RESOURCE_POOL_LIST` 将资源池分配给租户，可以通过 `DBA_OB_TENANTS` 视图查看所有租户。
<!-- 这里介绍的应该是在创建租户时分配资源池，为什么要在创建前查看所有租户
可以将资源池分配给已创建的租户么 -->
指定资源池时，可以每个 Zone 单独指定一个资源池，使用独立的资源规格；也可以所有 Zone 使用同一个资源池，从而所有 Zone 使用同一个资源规格。除了资源池列表，还有 Primary Zone、Locality、连接白名单等其他重要属性和系统变量，其中资源池列表为创建租户时的必填项。
...
<!-- 替换为 V4.2.1 的示例 -->
```bash
MySQL [oceanbase]> select * from dba_ob_tenants \G
*************************** 1. row ***************************
                 TENANT_ID: 1
               TENANT_NAME: sys
               TENANT_TYPE: SYS
