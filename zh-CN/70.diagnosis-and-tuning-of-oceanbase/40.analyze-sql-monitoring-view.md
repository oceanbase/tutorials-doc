...
![](https://intranetproxy.alipay.com/skylark/lark/0/2024/jpeg/132317/1708409456021-619b9030-7415-4611-8d91-66ebd50098bf.jpeg#clientId=uc537fae4-7412-4&from=paste&id=P8zIa&originHeight=1010&originWidth=2000&originalType=url&ratio=1.7999999523162842&rotation=0&showTitle=false&status=done&style=none&taskId=u41c6a963-4adf-40fb-9330-6750780fb28&title=) 监控指标相关的数据来源于 OceanBase 数据库内部的动态性能视图，所有监控指标都可以通过 SQL 语句进行访问。动态性能视图分为 GV$ 视图和 V$ 视图，外部监控系统（例如 OCP）通过在每个数据库服务器上部署代理进程，通过 SQL 接口定期拉取本机上的监控信息（V$ 视图），部分全局信息（例如 Root Service 相关）通过中心节点采集。监控数据统一汇报给监控系统数据库，并按照各种维度聚合（集群维度、租户维度、节点维度、Unit 维度），从而构建整个监控大盘。

**说明：**

1. 监控指标的数据来源于 OceanBase 数据库的视图，但日常工作中应该尽量使用可视化的外部监控系统，可以更加方便地分析监控指标的变化趋势，提高效率。
2. 由于 GV$ 视图和 V$ 视图的同质性，本小节不再专门对其进行区分，将根据具体场景选择合适的视图。
...
- enable_sql_audit：集群配置项，默认值为 True，控制全部租户的 SQL Audit 功能是否开启，动态生效。
...
-- 下面两个命令需要使用 sys 租户登录并执行，对整个集群生效
...
- ob_enable_sql_audit：租户级别的系统变量，默认值为 ON，控制当前租户是否开启 SQL Audit 功能，动态生效。
...
- ob_sql_audit_percentage：租户级别的系统变量，控制当前租户 SQL Audit 功能占用租户内存的百分比，默认值为 3（即百分之三），动态生效。为防止 SQL Audit 占用过高内存，系统设置其内存上限为 1GB。
...
此外，Sql Audit 支持手动淘汰，粒度为租户级别，淘汰方式如下：
...
一些重要的事件间隔
![](https://intranetproxy.alipay.com/skylark/lark/0/2023/jpeg/24156637/1696500547686-2337bd55-6818-4cf6-a0b9-58d587a30d34.jpeg)

## 相关问题

GV$OB_SQL_AUDIT 视图的完整字段介绍，详见：[官网链接](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000510560#2-title-GV$OB_SQL_AUDIT%20%E5%AD%97%E6%AE%B5%E8%AF%B4%E6%98%8E)。

## GV$OB_SQL_AUDIT 视图使用示例

通过 GV$OB_SQL_AUDIT 视图，我们可以方便的查询 SQL 执行的各种维度信息，下面就列举几个常用的相关 SQL。

- 慢 SQL 统计：查询一段时间内，耗时超过某个阈值的 SQL，后续可以根据查询的结果对特定 SQL 进行调优。

```sql
-- 这个例子中查询的是：tenant id 为 1002 的租户，在 2024-02-20 12:00:00 之后，query 执行时间超过 100 ms 的前五条 SQL
select
  tenant_id,
  request_id,
...
- 事务内涉及的 SQL 统计：sql_audit 里的每条 SQL 预期都记录了当前 SQL 所涉及的事务唯一标识 transaction_hash，可以根据该字段找到当前事务内所有的 SQL 信息，进而基于该信息确定业务压测的事务模型是否符合预期等。

```sql
-- 这个例子中查询的是：tenant id 为 1 的 sys 租户，在 tx_id = '27592485' 这个事务中，执行了哪些 query
select
  tenant_id,
  tx_id,
  query_sql
...
- 同一个 Session 上执行的 SQL：基于 SID 可以查询出某一个 Session 上所有的业务请求，通常被用来分析业务模型。

```sql
-- 这个例子中查询的是：tenant id 为 1002 的租户，sid = 3221652146 这个 session 中，在 2024-02-20 12:00:00 之后，执行过哪些 query
select
  tenant_id,
  sid,
  query_sql
...
- 查询占用租户资源最多的 SQL，按照 执行时间 * 执行次数 降序排序。如果该租户当前存在容量不足（租户 CPU 使用率被打爆），可以通过该语句判断是否为 SQL 问题，及具体的疑点 SQL 是什么。
...
在反复执行同一条相同的 SQL 时，执行计划可能会随着一些信息的变更而发生变化，例如数据库对象的变更、统计信息的变更、OBServer 版本的变更等等。 **所以为了确认某一条 SQL 在执行时的真实计划，不应该直接执行 explain 语句，而是要通过查询上述的两个计划监控视图 gv$ob_sql_audit 和 GV$OB_PLAN_CACHE_PLAN_EXPLAIN。**
下面通过举例说明如何通过这两个计划监控视图确认具体某一条 SQL 在执行时的真实计划是什么：

```sql
-- 这个例子展示的就是如何准确地查询 select * from oceanbase.DBA_OB_DATABASES; 这条 SQL 在执行时的真实计划
...
-- 通过 oceanbase.gv$ob_sql_audit 查询上述 SQL 对应的 TENANT_ID、SVR_IP、SVR_PORT、PLAN_ID 字段
-- 因为后面查询 GV$OB_PLAN_CACHE_PLAN_EXPLAIN 视图时 , 需要指定 TENANT_ID、SVR_IP、SVR_PORT、PLAN_ID 的等值条件，否则查询结果为空
select TENANT_ID,
  SVR_IP,
  SVR_PORT,
  PLAN_ID,
...
-- 指定 TENANT_ID、SVR_IP、SVR_PORT、PLAN_ID 的等值条件，查询 GV$OB_PLAN_CACHE_PLAN_EXPLAIN 视图，即可得到对应 SQL 在执行时的真实计划
SELECT
  *
FROM
...
最后再介绍一个 GV$OB_SQL_PLAN 系统视图，这个视图可以展示计划在第一次执行时真实的执行反馈信息，例如会同时展示优化器估算当前算子的输出行数，以及计划第一次执行时当前算子的真实输出行数。通过对比这些数值，可以反映出优化器的代价评估是否存在明显的问题。下面举一个简单的例子：
...
GV$OB_SQL_PLAN 系统视图的介绍详见：[官网链接](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000508691)。
