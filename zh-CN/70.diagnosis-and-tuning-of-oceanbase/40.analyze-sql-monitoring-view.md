...
![视图](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/40.analyze-sql-monitoring-view-01.jpeg)
...
本小节主要介绍的监控手段均属于 SQL 监控，即根据特定 SQL 的多次执行或特定执行中收集的监控信息对系统的性能问题进行诊断。系统监控部分的内容详见官网《OceanBase 数据库》文档 [管理数据库/监控指标/监控指标/系统监控](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000508297) 章节。
...
`GV$OB_SQL_AUDIT` 视图的字段很多，我们在本小节中只介绍其中一部分比较重要的字段。`GV$OB_SQL_AUDIT` 视图的完整字段介绍，详见官网《OceanBase 数据库》文档 [参考指南/性能调优/SQL 调优指南/SQL 优化/SQL 执行性能监控](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000510560) 中 **GV$OB_SQL_AUDIT 字段说明**。
...
![事件间隔](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/40.analyze-sql-monitoring-view-02.jpeg)
...
- 查询占用租户资源最多的 SQL
  ...
  通过上面这条 SQL 进行问题定位的案例，详见官网《OceanBase 数据库》文档 [管理数据库/监控指标/案例](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000507624)。

## 执行计划监控视图

OceanBase 数据库为了避免反复对同一条 SQL 生成执行计划，会将生成的计划放进 Plan Cache，再次对相同的 SQL 生成计划时，会直接从 Plan Cache 获取之前已经生成好的 Plan。每个租户在每个节点上都有一个独立的 Plan Cache，用以缓存在此节点上处理过的 SQL Plan。
...
最后再介绍一个 `GV$OB_SQL_PLAN` 系统视图，这个视图可以展示计划在第一次执行时真实的执行反馈信息，例如会同时展示优化器估算当前算子的输出行数，以及计划第一次执行时当前算子的真实输出行数。通过对比这些数值，可以反映出优化器的代价评估是否存在明显的问题。`GV$OB_SQL_PLAN` 系统视图的介绍详见官网《OceanBase 数据库》文档 [参考指南/系统试图/MySQL 租户系统视图/性能视图/GV$OB_SQL_PLAN](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000219887)。
