...
OceanBase 数据库是原生分布式数据库系统，故障根因分析通常是比较繁琐的，因为涉及的因素可能有很多，如机器环境、配置参数、运行负载等等。专家在排查问题的时候需要获取大量的信息来分析故障，如何高效的获取故障场景下分散在各个节点的信息，挖掘出其中的关联性便是OceanBase诊断工具(OceanBase Diagnostic Tool) ，简称 obdiag 需要解决的问题。

![image.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/90.diagnose-and-analyze-through-obdiag-01.png)
...
![image.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/90.diagnose-and-analyze-through-obdiag-02.png)
...
![image.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/90.diagnose-and-analyze-through-obdiag-03.png)
...
- 离线部署(不可访问外网的情况下可选择) OceanBase官网 [下载页](https://www.oceanbase.com/softwarecenter) 下载新版的 OceanBase 数据库定制的敏捷诊断工具（obdiag）。
...
- debian系（如 Ubuntu）部署 OceanBase官网 [下载页](https://www.oceanbase.com/softwarecenter) 下载新版的 OceanBase 数据库定制的敏捷诊断工具（obdiag）。
...
具体编写教程参见官网文档：[https://www.oceanbase.com/docs/common-obdiag-cn-1000000000691956](https://www.oceanbase.com/docs/common-obdiag-cn-1000000000691956)
...
诊断信息收集的场景 yaml 增加和本教程 [一键巡检场景添加](#一键巡检场景添加) 的巡检场景类似，本节不做赘述，下面给个例子。
...
- info_en：英文描述，在obdiag gather scene list 的时候展示的英文描述就出自于此；

- info_cn:  中文描述，在obdiag gather scene list 的时候展示的中文描述就出自于此；
...
- task.steps.type.global: 相对于节点来说是否是全局采集项，global 值为 true 的只会在第一个节点上执行一次，global 为 true 的会在每个节点上都执行。

### 根因分析场景添加

根因分析场景的目录为 `~/.obdiag/rca`，展开后里边是 python 文件，根因分析场景的添加稍微复杂一点，需要在 python 代码中处理根因分析的具体链路逻辑，更多详情参见 obdiag 开发者文档：[https://oceanbase.yuque.com/org-wiki-obtech-vh7w9r/imzr6c/or82ioetg06ta27e](https://oceanbase.yuque.com/org-wiki-obtech-vh7w9r/imzr6c/or82ioetg06ta27e)
...
![image.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/tutorials/V4.x/70.diagnosis-and-tuning-of-oceanbase/90.diagnose-and-analyze-through-obdiag-04.png)
