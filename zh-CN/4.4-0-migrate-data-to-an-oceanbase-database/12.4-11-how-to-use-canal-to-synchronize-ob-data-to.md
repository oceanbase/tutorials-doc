...
* 确认 oblogproxy 依赖，重点是确认有 liboblog。
    ...
    ```bash
    [root@192.168.0.1 ~]$cd /usr/local/oblogproxy/

    [root@192.168.0.1 oblogproxy]$ldd ./bin/logproxy
...
* 生成加密用户和对应的密码
  ...
  ```bash
  [root@192.168.0.1 bin]$pwd
  /usr/local/oblogproxy/bin

  [root@192.168.0.1 bin]$./logproxy -x root@sys
  EA87898018FD1EDDC2AA11CE1556E917

  [root@192.168.0.1 bin]$./logproxy -x Root@2021
...
* 修改对应的 conf.json 参数
  ...
  ```bash
  [root@192.168.0.1 conf]$pwd
  /usr/local/oblogproxy/conf

  [root@192.168.0.1 conf]$ls -l
  总用量 4
...
* 启动 oblogproxy
  ...
  ```bash
  [root@192.168.0.1 oblogproxy]$./run.sh start
...
* 启停 obproxy 和查看状态
  ...
  * 启动 oblogproxy
    ...
    ```bash
    [root@192.168.0.1 oblogproxy]$pwd
  ...
  * 停止 oblogproxy
    ...
    [root@192.168.0.1 oblogproxy]$pwd
    /usr/local/oblogproxy
    [root@192.168.0.1 oblogproxy]$./run.sh stop
    ```
  ...
  * 查看 oblogproxy 状态
    ...
    [root@192.168.0.1 oblogproxy]$pwd
    /usr/local/oblogproxy
    [root@192.168.0.1 oblogproxy]$./run.sh status
    ```
...
```bash
[root@192.168.0.1 ~]$ps -ef | grep logproxy | grep -v grep
...
```bash
[root@192.168.0.1 ~]$ps -ef | grep oblogreader | grep -v grep
...
```yaml
canal.zkServers = 192.168.0.2:12181,192.168.0.3:12181,192.168.0.1:12181
...
```bash
[root@192.168.0.1 obtest2]$pwd
/opt/canal_ob/canal.deployer-for-ob-rc1/conf/obtest2
[root@192.168.0.1 obtest2]$ll
total 16
...
[root@192.168.0.1 obtest2]$cat instance.properties
```

```yaml
# oceanbase集群参数
canal.instance.oceanbase.rsList=192.168.0.1:2882:2881;192.168.0.2:2882:2881
...
```bash
[root@192.168.0.1 ~]$cd /opt/canal_ob
[root@192.168.0.1 canal_ob]$cd canal.deployer-for-ob-rc1
[root@192.168.0.1 canal.deployer-for-ob-rc1]$cd bin
[root@192.168.0.1 bin]$ls -lrt
总用量 20
...
[root@192.168.0.1 bin]$./startup.sh
```
...
[root@192.168.0.1 conf]$cat application.yml
```
      ...
      - groupId: g1
        ...
        - name: rdb
          ...
          properties:
            ...
            jdbc.url: jdbc:mysql://192.168.0.1:3367/mysql?useUnicode=true&useSSL=false
            jdbc.username: root
  ...
  [root@192.168.0.1 conf]$pwd
  /opt/canal_ob/canal_adapter_for_ob/conf
  [root@192.168.0.1 conf]$ls -l rdb
  总用量 4
