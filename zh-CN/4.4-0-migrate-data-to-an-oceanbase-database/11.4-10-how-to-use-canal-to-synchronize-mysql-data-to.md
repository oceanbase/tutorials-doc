...
对于自建 MySQL , 需要先开启 Binlog 写入功能，配置 `binlog-format` 为 ROW 模式，配置后需重启 MySQL 生效。`/etc/my.cnf` 中配置如下：
...
然后初始化一个业务数据库（TPCH，后面介绍性能测试的时候再重点介绍）。
...
* 修改配置
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>passwd</code> 后面的字符串是 <code>admin</code> 在 MySQL 里的密文。这个密码跟前面 Canal Admin 配置文件里的密码保持一致。如果前面密码改了，这里也要相应修改，密文的值可以通过 MySQL 的 <code>password</code> 方法获取。</p>
  </main>
...
* （可选）图形化部署 Canal server 和 Canal instance
  ...
  如果使用 Canal Admin 管理 Canal server，则登录 admin 的管理界面 <http://172.24.50.39:8089/#/canalServer/nodeServers>，单机情况下，启动 Cananl Server 后，Canal Admin 的 **server 管理** 页面会自动创建一个 Server。您也可以选择 **Canal Server** > **Server 管理**，单击 **新建Server**。

  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>此处链接中的 IP 为示例中配置 Canal Admin 的服务器 IP，仅供参考。您需根据实际情况将其转换为自身配置 Canal Admin 的服务器 IP。</p>
  </main>

  ![Server 管理](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p367320.jpg)

  在弹出的 **新建Server信息** 界面填写相关信息后单击 **确定** 按钮。
...
* 修改启动器配置：`adapter/conf/application.yml`，这里以 OceanBase 目标库为例。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>如果没有用 Canal Admin 部署，沿用的是 <code>example</code> 这个名称；如果用了 Canal Admin 部署 <code>instance</code>，前面命名的是 <code>mariadb</code>。</p>
  </main>
...
* RDB 映射文件
  ...
  修改 `adapter/conf/rdb/mytest_user.yml` 文件。
