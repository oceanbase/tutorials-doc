...
MySQL 兼容性主要和表的数据类型、业务 SQL 的兼容性有关。 MySQL 的函数、触发器、存储过程在 OceanBase 的 MySQL 模式中支持的并不好，也不推荐用。所以本章数据迁移只包含数据库表对象及其数据的迁移。
...
与 MySQL 数据库相比，OceanBase 数据库暂不支持空间数据类型和 JSON 数据类型，其他类别的数据类型除了大对象外，支持情况等于或大于 MySQL 数据库。
...
MySQL 的部分变量在 OceanBase 的 MySQL 租户中同样适用。但是注意源端 MySQL 和目标端 OceanBase 变量值的差异。
...
OceanBase MySQL 租户支持字符集 `binary` 和 `utf8mb4`。 默认字符集是 `utf8mb4`。租户的字符集在租户创建时指定，是租户全局级别的字符集。字符集 `utf8mb4` 是 `utf8` 的超集，多一些表情字符的编码。建议迁移到 OceanBase 使用 `utf8mb4` 字符集。 在 `utf8mb4` 字符集下，每个英文字符长度 1 字节，每个汉字长度 3 字节，每个表情字符长度 4 字节。如果迁移数据更换了字符集，字符串列的长度需要适当放长。
...
从 MySQL 中导出数据时，请确保数据库里的字符都能正确输出到文件中。通过 `vim` 命令下的 `:set fileencoding` 命令可以查看文件的编码。一般都是 `utf-8`。这样通过文件迁移 MySQL 数据时不会出现乱码现象。
