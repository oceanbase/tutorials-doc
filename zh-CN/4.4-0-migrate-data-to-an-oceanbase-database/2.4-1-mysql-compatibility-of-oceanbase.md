...
OceanBase MySQL 租户支持字符集 `binary` 和 `utf8mb4`。 默认字符集是 `utf8mb4`。租户的字符集在租户创建时指定，是租户全局级别的字符集。字符集 `utf8mb4` 是 `utf8` 的超集，多一些表情字符的编码。

迁移数据到 OceanBase 时建议使用 `utf8mb4` 字符集。 在 `utf8mb4` 字符集下，每个英文字符长度为 1 字节，每个汉字长度为 3 字节，每个表情字符长度为 4 字节。如果迁移数据更换了字符集，字符串列的长度需要适当增加。
