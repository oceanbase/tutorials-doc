...
将 ZONE2 的 REGION 属性和 ZONE1 的 REGION 属性调整一致后，即可设置 PRIMARY_ZONE 为 `zone1` 或 `zone2`。并且 `zone3` 不再出现在候选里了，因为 `zone3` 的 `region3` 里只有一个 FULL 副本，不具备设置为 PRIMARY_ZONE 的资格。
...
OceanBase 数据库默认是强一致性读，即：写后读立即可见（READ AFTER WRITER）。 使用强一致性读策略时，OBPROXY 会优先路由到访问表的分区的主副本节点上。
