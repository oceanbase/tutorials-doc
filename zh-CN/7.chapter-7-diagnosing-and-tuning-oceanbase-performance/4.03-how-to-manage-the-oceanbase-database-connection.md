...
由于执行计划跟连接方法也有关系，这里先介绍一下 OceanBase 数据库的连接原理。
...
通常我们把客户端跟 OBPROXY 建立的连接称为"前端连接"，把相应的 OBPROXY 跟后端 OBSERVER 节点之间的连接称之为"后端连接"。每个前端连接可能对应 1-N 个后端连接。 N 是 OceanBase 租户所在的节点总数（包括备副本所在 OBSERVER 节点）。不同前端连接对应的后端连接是不复用的。
...
后端连接被 KILL ，客户端是感知不到的，再次查询的时候 OBPROXY 会自动建立新的后端连接。
...
* `Proxy_sessid` ：前端连接的 OBPROXY 内部 ID，该 OBPROXY 内部唯一，不同 OBPROXY 的 

  `Proxy_sessid` 会重复。如果为 NULL ，表示是直连 OBSERVER 建立的连接。
...
说明：`Host`：客户端连接信息（`IP:PORT`）。通常就是 OBPROXY 地址或者发起连接的客户端。
...
为了研究这两个参数的影响， 以下示例故意把两个参数的值差异拉大。
