...
* 通过 ODP 的 2883 端口连接 OceanBase 集群。ODP 和 OceanBase 集群保持联系，能感知集群节点状态变化。您可以使用多个 ODP 连接 OceanBase 集群。ODP 之间彼此独立工作，互不影响。

## ODP 连接原理

ODP 是个单进程程序，默认监听端口 2883，实现了 MySQL 连接协议。客户端和 ODP 建立连接后，访问 OceanBase 集群数据时，ODP 会自动判断要访问的数据的主副本在哪个 OBServer 节点上，然后自动和该 OBServer 节点建立一个长连接。

通常我们把客户端和 ODP 建立的连接称为前端连接，把相应的 ODP 和后端 OBServer 节点之间的连接称之为后端连接。每个前端连接可能对应 1-N 个后端连接。N 是 OceanBase 租户所在的节点总数（包括备副本所在 OBServer 节点）。不同前端连接对应的后端连接是不复用的。

![18](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7353672461/p384291.jpeg)

ODP 前端连接数受 ODP 参数 `max_connections` 和 `client_max_connections` 限制。
...
* 查看并直接 KILL 前端连接
  ...
  `show processlist` 只能查看当前客户端连接，`show proxysession` 只能查看当前 ODP 的连接。
...
* 查看并直接 KILL 后端连接
  ...
  * {`proxy_sessid`、`cs_id`、`host_ip`、`host_port`｝为前端连接元组。其中 `cs_id` 是 ODP 内部标识的前端连接（客户端连接）的 ID 号，跟 `show processlist` 的 ID 列一致。

  * {`server_ip`、`server_port`、`server_sessid`、`ss_id` } 组成后端连接元组。其中 `ss_id` 是 ODP 内部标识的后端连接（ODP 跟 OBServer 节点连接）的 ID 号。`server_sessid` 是 OBServer 节点上的客户端连接 ID。
...
### 通过 ODP 连接 OceanBase 集群管理连接

使用 ODP 连接 OceanBase 集群。

```bash
obclient -h172.20.249.52 -uroot@sys#obce-3zones -P2883 -p0******d -c -A oceanbase
```

通过 ODP 连接时，`show processlist` 和 `show proxysession` 只能查看当前 ODP 的客户端连接，`show proxysession` 能看到其他 OceanBase 集群的连接（如果这个 ODP 还可以为其他集群提供路由服务），`show full processlist` 能看到 OceanBase 集群的全部后端连接，包括通过其他 ODP 的连接。

在这个连接里可以针对性的 KILL 当前 ODP 的后端连接（不能 KILL 其他 ODP 的后端连接），但是前端连接只能 KILL 自己，不能 KILL 其他连接。支持 KILL CONNECTION 和 KILL QUERY。
...
客户端感知不到后端连接被 KILL。再次查询时 ODP 会自动建立新的后端连接。

### 直连 OBServer 节点管理连接

在 ODP 的连接里查看后端连接的命令是 `show full processlist`，输出的 `Id` 列就是 OBServer 节点里客户端连接 ID。可以在 OceanBase 数据库内部直接使用 KILL QUERY 或 KILL CONNECTION。
...
* `Proxy_sessid`：前端连接的 ODP 内部 ID，该 ID 在 ODP 内部唯一，不同 ODP 的 `Proxy_sessid` 会重复。如果为 NULL，表示是直连 OBServer 节点建立的连接。
  
如果您直连到 OBServer 节点，执行以上命令，查询结果会有点不一样。
...
说明：`Host` 客户端连接信息（`IP:PORT`），通常为 ODP 地址或者发起连接的客户端。
...
* `Host`：前端连接的客户端，即发起连接的客户端信息（`IP:PORT`）。通常为 ODP 地址或者发起连接的客户端。

* `Ip`：后端连接的服务端地址，通常为 OBServer 节点地址。

* `Id`：后端连接的 ID 标识，整个 OceanBase 集群内部唯一。可以直接 KILL QUERY 或 KILL CONNECTION。

直连 OBServer 节点不支持 ODP 的管理命令。
...
跟前面一样，如果后端连接被 KILL，客户端重新查询时，ODP 将自动和 OBServer 节点建立后端连接。

## 视图 __all_virtual_processlist

视图 `__all_virtual_processlist` 能查看到 OceanBase 集群的所有连接，包括通过 ODP 连接的后端连接以及直连集群 OBServer 节点的连接。
...
* `proxy_sessid`：如果为 `NULL`，表示直连 OBServer 节点的连接；如果不为 NULL，表示通过 ODP 建立的连接。

* `host`：连接的客户端。通常是 ODP 地址，如果是直连，则为发起连接的实际客户端地址。
...
* `trace_id`：连接使用的 TRACE_ID，可以在 OBServer 节点的运行日志中根据 TRACE_ID 追踪相关日志。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>不管是哪种超时，连接事务所持有的锁都会释放，只是事务的状态需要客户端主动清理一下。对于应用而言，就是需要捕捉事务超时报错，然后发起 ROLLBACK 。</p>
</main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>不管是哪种超时，连接事务所持有的锁都会释放，只是事务的状态需要客户端主动清理。对于应用而言，就是需要捕捉事务超时报错，然后发起 ROLLBACK。</p>
</main>
