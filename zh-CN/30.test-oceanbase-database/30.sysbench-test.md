...
本文基于 x86 架构的 CentOS Linux 7.9 镜像环境，介绍两种方式对 OceanBase 数据库运行 Sysbench 测试：
...
3. 编译 Sysbench
   ...
   [admin@test sysbench-1.0.20]$ make install
   [admin@test sysbench-1.0.20]$ cp -r /usr/sysbench/share/sysbench/* /usr/sysbench/bin/
   ```
...
4. 验证是否安装成功
   ...
   ```shell
   [admin@test sysbench-1.0.20]$ ./src/sysbench --help
   ```
...
> **说明**
>
> 若要在系统租户中修改 OBProxy 配置项，您需使用 OBProxy 节点 IP 和端口登录 OceanBase 数据库。
...
在 Sysbench 压测过程中，鉴于 OceanBase 数据库的安装配置、业务租户的创建以及 Sysbench 工具的使用细节等可能因环境差异而有所不同，可能出现实际测试结果不符合预期的情况。我们也提供一些性能问题的案例作为参考，来帮助大家解决可能遇到的性能问题。详细案例可参考 OceanBase 官网《OceanBase 数据库》文档中 [参考指南/性能测试/OceanBase Sysbench 高性能部署和问题分析](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000218150) 一文。
