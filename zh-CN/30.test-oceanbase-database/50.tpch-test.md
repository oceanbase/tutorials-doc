...
<!-- tpch 里没有 obdiag 内容的介绍了，tpch 中不需要先进行巡检么 -->
...
2. 执行测试
   ...
   * `obd test tpch` 命令会自动完成所有操作，包含测试数据的生成、性能参数调优、数据导入和测试。命令的详细介绍可参见官网文档《OceanBase 安装部署工具》中 [OBD 命令/测试命令组](https://www.oceanbase.com/docs/community-obd-cn-1000000000774252) 中 **obd test tpch**。
...
2. 解压 TPC-H Tool
   ...
   ```shell
   [admin@test ~]$ unzip 7e965ead-8844-4efa-a275-34e35f8ab89b-tpc-h-tool.zip
   ```

3. 修改 Makefile 文件

   执行如下命令复制 `makefile.suite` 文件并重命名为 `Makefile`
   <!-- 需确认 cp 命令是在哪个目录下执行的 -->
   ```shell
   [admin@test ~]$ cd TPC-H_Tools_v3.0.0/dbgen
...
4. 修改 tpcd.h 文件
   ...
   修改后内容如下：
   <!-- 这里展示的是我们进行测试时手动添加的信息么，还是整个文件就这么多信息 -->
   ```shell
   #ifdef MYSQL
...
您可参考本节中的下述步骤生成查询 SQL 后进行调整，也可直接使用 [GitHub](https://github.com/oceanbase/obdeploy/tree/master/plugins/tpch/3.1.0/queries) 中给出的查询 SQL。若您选择使用 GitHub 中的查询 SQL，需将 SQL 语句中的 cpu_num 修改为实际并发数。
...
1. 创建加载脚本目录
   ...
   [admin@localhost load] $ cp xx/create_tpch_mysql_table_part.ddl ./
   ```
...
3. 加载数据
   ...
   (0, 'obclient: [Warning] Using a password on the command line interface can be insecure.')
   ```
   <!-- 命令里 （0，xxxx 信息是执行 pytho 命令的输出么 -->

4. 执行合并
   
   <!-- 执行合并操作需登录 sys 租户。--root@sys 用户么，还是仅要求 sys 租户即可 -->
...
5. 查看合并是否完成
   ...
   ```shell
   obclient [oceanbase]> SELECT FROZEN_SCN, LAST_SCN FROM oceanbase.CDB_OB_MAJOR_COMPACTION;
   ```
