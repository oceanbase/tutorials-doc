...
本文中我们一共介绍两种方式对 OceanBase 数据库运行 TPC-H 测试：
...
<!-- tpcc 里没有 obdiag 内容的介绍了，tpcc 中不需要先进行巡检么 -->
...
* part：表示零件信息，主键为 `p_partkey`，取值范围 1~SF*200000，与 partsupp 关联。
...
* mysql-devel：可执行 `yum install mysql-devel` 安装
...
● tpch tool：下载地址 https://www.tpc.org/tpc_documents_current_versions/download_programs/tools-download-request5.asp?bm_type=TPC-H&bm_vers=3.0.0&mode=CURRENT-ONLY

<!-- > **注意**
>
> 当使用的 OBClient 版本大于等于 2.2.0 时，会默认开启 ob20 协议以及全链路追踪的能力，在 Sysbench 测试中会影响性能，建议通过设置环境变量手动关闭：export ENABLE_PROTOCOL_OB20=0。 -->
<!-- tpcc 中没有这个说明，开启 20 协议不影响 tpcc 性能测试么-->
...
2. 执行测试
   ...
   命令中的 deploy_name 表示部署集群名，tenant_name 表示进行测试的租户名（即 [租户规格](#租户规格) 中的 `tpcc_tenant`），您需根据实际部署集群名和租户名进行修改。`obd test tpcc` 命令会自动完成所有操作，包含测试数据的生成、性能参数调优、数据导入和测试。命令的详细介绍可参见官网文档《OceanBase 安装部署工具》中 [OBD 命令/测试命令组](https://www.oceanbase.com/docs/community-obd-cn-1000000000774252) 中 **obd test tpcc**。
...
5. 在 benchmarksql-5.0/run 目录下，创建文件 prop.oceanbase。
   ...
   * 并发数量（terminals）：800，MySQL 租户配置下并发需要结合具体配置动态调整。取值须在 (0, 10*numWarehouses] 范围内。
   ...
   * numTerminals > 0 && numTerminals <= 10*numWarehouses，terminals 的范围需要在这个区间内。
     <!-- numWarehouses 是什么，之前没有出现过，numTerminals 之前也没有出现过
     这个是对 terminals 的限制么，是否可以放到 并发数量 的说明里 -->
...
3. 执行合并
   ...
   `<your tenant name>` 表示xxxxx，您需根据实际测试租户进行替换。
   <!-- 表示待执行测试的租户名？ -->
...
4. 查看合并是否完成
   ...
   输出如下，所有的 FROZEN_SCN 和 LAST_SCN 的值相等即表示合并完成。
...
5. 手动收集统计信息
   ...
   执行 `obclient -h<host_ip> -P<host_port> -u<user_name>@<tenant_name> -p -A -D<db_name>` 命令进入测试租户，执行如下命令。

   ```sql
   call dbms_stats.gather_schema_stats('$db_name',degree=>96);
   ```
   <!-- 直接复制执行就行么，需要将 db_name 替换为一个什么数据库名么 -->
...
性能测试数据受到多种因素的影响，包括硬件配置、数据库安装部署方式、业务租户资源分配等，实际性能数据可能因环境因素会有些差异，此处测试结果仅供参考。
