...
```shell
find ~/.obd/plugins/${component} "parameter.yaml"
```
...
2. 修改配置文件
   ...
   调整集群资源相关参数时，需注意调大参数数值时，不能超出服务器剩余资源范围；调小参数数值时，不能低于租户已分配占用的数值范围。资源参数的调整之间有关联关系，具体可参见 [部署前准备](../20.deploy-oceanbase-database/10.preparation-before-deployment.md) 中 **OceanBase 数据库常用资源参数简介及计算方式** 一节。
...
* 扩容前，扩容的节点建议先进行服务器初始化，其中部署用户和 SSH 连接信息需要确保一致。详细介绍可参见本教程第二章 [部署前准备](../20.deploy-oceanbase-database/10.preparation-before-deployment.md)。
