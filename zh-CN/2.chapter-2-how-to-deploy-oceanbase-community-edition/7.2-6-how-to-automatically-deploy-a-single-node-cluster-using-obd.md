...
这里使用 LVM 技术对 `/dev/vdb` 进行划分。使用 LVM 技术划分 LV 大小时请根据实际磁盘大小调整参数。
...
后续版本的配置文件格式可能会变化，具体情况请参考 [配置文件说明](t2159894.html#topic-2159894)。
...
部署成功后，OBD 会把配置文件 `obce-single.yaml` 复制到自己的工作目录里（`~/.obd/cluster/obce-single/config.yaml` ），后期再对 `obce-single.yaml` 文件进行修改是不生效的。
**说明**
...
* 检查进程
  ...
  进程启动时，通过 `-o` 指定了很多参数，这些参数都是在前面 OBD 集群部署配置文件里指定的。
...
和传统 MySQL 不同的是 OBSERVER 连接端口是 `2881`， 连接用户名是 `root@sys`，密码则在前面 OBD 配置文件里指定。
