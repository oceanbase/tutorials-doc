...
OBAgent 部署配置文件可以跟 OceanBase 集群部署配置文件部署在一起，也可以后期单独部署。附录 [A.1](#sectiondiv-061-tpb-v6y) 展示了同时部署 OceanBase 集群和 OBAgent。

下面示例是采用单独的配置文件部署 OBAgent 。OBAgent 部署配置文件的步骤跟 OceanBase 集群部署配置文件一样。

1. 指定部署节点，包括节点名称和 IP 。节点名称保持唯一就行，可以是主机名（假设主机名是唯一的）。
...
第一次使用 `deploy` 命令时需指定 OBAgent 的配置文件。
...
OBAgent 启动后会在节点自动生成 `Prometheus` 配置文件，该配置文件被放在 OBAgent 安装目录下，如 `/home/admin/obagent/conf/prometheus_config/`。这个配置文件可以给 `Prometheus` 产品直接使用。
...
启动后可通过浏览器进行访问：http://172.24.50.39:9090/graph 。
...
如果 `OBAgent` 是跟 `OceanBase` 一起部署的，则只能重启组件 `obagent`：
