...
|                   软件包名                   |   进程名    |                软件用途                |
|------------------------------------------|----------|------------------------------------|
| oceanbase-ce-*x.x.x*-1.el7.x86_64.rpm      | observer | OceanBase 数据库进程，常驻后台运行。            |
| oceanbase-ce-libs-*x.x.x*-1.el7.x86_64.rpm |     --   | 提供软件运行的 library，不运行。               |
| obproxy-*x.x.x*-1.el7.x86_64.rpm           | obproxy  | OceanBase 访问反向代理，单进程，常驻后台运行。       |
| ob-deploy-*x.x.x*-1.el7.x86_64.rpm             | obd      | OceanBase 自动化部署软件，提供部署命令行，不常驻后台运行。 |
|libobclient-*x.x.x*-2.el7.x86_64.rpm|--|obclient 依赖包。|
| obclient-*x.x.x*-2.el7.x86_64.rpm          | obclient | OceanBase 官方命令行客户端。                 |
