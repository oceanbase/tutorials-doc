...
|    角色    |      机器       |         备注          |
|----------|---------------|---------------------|
| OBD      | 192.168.0.2 | 中控机，自动化部署软件         |
| OBServer | 192.168.0.4 | OceanBase 数据库 zone1 |
| OBServer | 192.168.0.1 | OceanBase 数据库 zone2 |
| OBServer | 192.168.0.3 | OceanBase 数据库 zone3 |
| OBProxy  | 192.168.0.2 | OceanBase 访问反向代理    |
| OBClient | 192.168.0.2 | OceanBase 命令行客户端    |
...
```bash
[admin@obce02 oceanbase]$ sudo clockdiff 192.168.0.4
[sudo] password for admin:
.
host=192.168.0.4 rtt=750(187)ms/0ms delta=0ms/0ms Sun Sep 12 14:52:24 2021
[admin@obce02 oceanbase]$ sudo clockdiff 192.168.0.3
.
host=192.168.0.3 rtt=750(187)ms/0ms delta=0ms/0ms Sun Sep 12 14:52:30 2021
```

有些机器使用 `clockdiff` 可能会报错。此时您可使用以下命令判断时间同步误差。

```bash
[admin@obce02 oceanbase]$ ping -T tsandaddr 192.168.0.4 -c 2
PING 192.168.0.4 (192.168.0.4) 56(124) bytes of data.
64 bytes from 192.168.0.4: icmp_seq=1 ttl=64 time=0.161 ms
TS:     192.168.0.1   24851014 absolute
        192.168.0.4   -1
        192.168.0.4   0
        192.168.0.1   1

64 bytes from 192.168.0.4: icmp_seq=2 ttl=64 time=0.172 ms
TS:     192.168.0.1   24852054 absolute
        192.168.0.4   -1
        192.168.0.4   0
        192.168.0.1   1
```
...
> **说明**
>
> 示例中的安装包可能不是最新版本，建议下载最新的安装包，详细信息请参考 [OceanBase 发布说明](https://github.com/oceanbase/oceanbase/releases)。
...
* `192.168.0.4`
  ...
  cd ~/oceanbase && bin/observer -i eth0 -p 2881 -P 2882 -z zone1 -d ~/oceanbase/store/obdemo -r '192.168.0.4:2882:2881;192.168.0.1:2882:2881;192.168.0.3:2882:2881' -c 20210912 -n obdemo -o "memory_limit=8G,cache_wash_threshold=1G,__min_full_resource_pool_memory=268435456,system_memory=3G,memory_chunk_cache_size=128M,cpu_count=16,net_thread_count=4,datafile_size=50G,stack_size=1536K,config_additional_dir=/data/obdemo/etc3;/redo/obdemo/etc2" 
  ```
...
* `192.168.0.1`
  ...
  cd ~/oceanbase && bin/observer -i eth0 -p 2881 -P 2882 -z zone2 -d ~/oceanbase/store/obdemo -r '192.168.0.4:2882:2881;192.168.0.1:2882:2881;192.168.0.3:2882:2881' -c 20210912 -n obdemo -o "memory_limit=8G,cache_wash_threshold=1G,__min_full_resource_pool_memory=268435456,system_memory=3G,memory_chunk_cache_size=128M,cpu_count=16,net_thread_count=4,datafile_size=50G,stack_size=1536K,config_additional_dir=/data/obdemo/etc3;/redo/obdemo/etc2"
  ```
...
* `192.168.0.3`
  ...
  cd ~/oceanbase && bin/observer -i eth0 -p 2881 -P 2882 -z zone3 -d ~/oceanbase/store/obdemo -r '192.168.0.4:2882:2881;192.168.0.1:2882:2881;192.168.0.3:2882:2881' -c 20210912 -n obdemo -o "memory_limit=8G,cache_wash_threshold=1G,__min_full_resource_pool_memory=268435456,system_memory=3G,memory_chunk_cache_size=128M,cpu_count=16,net_thread_count=4,datafile_size=50G,stack_size=1536K,config_additional_dir=/data/obdemo/etc3;/redo/obdemo/etc2"
  ```
...
当 OceanBase 集群三个节点都正常启动，并且监听正常时，您可连接到任一节点（通过 `2881` 端口直连），进行自举（`bootstrap` 集群初始化）操作。初始密码为空。

```bash
mysql -h 192.168.0.1 -u root -P 2881 -p -c -A
```

```bash
set session ob_query_timeout=1000000000; alter system bootstrap ZONE 'zone1' SERVER '192.168.0.4:2882', ZONE 'zone2' SERVER '192.168.0.1:2882', ZONE 'zone3' SERVER '192.168.0.3:2882' ;
```

输出：

```bash
[admin@obce00 ~]$ mysql -h 192.168.0.1 -u root -P 2881 -p -c -A
Enter password:
...
MySQL [(none)]> set session ob_query_timeout=1000000000; alter system bootstrap ZONE 'zone1' SERVER '192.168.0.4:2882', ZONE 'zone2' SERVER '192.168.0.1:2882', ZONE 'zone3' SERVER '192.168.0.3:2882' ;
Query OK, 0 rows affected (0.001 sec)

Query OK, 0 rows affected (28.839 sec)

MySQL [(none)]> Bye
[admin@obce00 ~]$ mysql -h 192.168.0.1 -u root@sys -P 2881 -p -c -A
Enter password:
Welcome to the MariaDB monitor.  Commands end with ; or \g.
...
```bash
cd ~/obproxy-3.1.0/ && bin/obproxy -r "192.168.0.4:2881;192.168.0.1:2881;192.168.0.3:2881" -p 2883 -o "enable_strict_kernel_release=false,enable_cluster_checkout=false,enable_metadb_used=false" -c obdemo
```

输出：

```bash
[admin@obce00 obproxy-3.1.0]$ cd ~/obproxy-3.1.0/ && bin/obproxy -r "192.168.0.4:2881;192.168.0.1:2881;192.168.0.3:2881" -p 2883 -o "enable_strict_kernel_release=false,enable_cluster_checkout=false,enable_metadb_used=false" -c obdemo
bin/obproxy -r 192.168.0.4:2881;192.168.0.1:2881;192.168.0.3:2881 -p 2883 -o enable_strict_kernel_release=false,enable_cluster_checkout=false,enable_metadb_used=false -c obdemo
rs list: 192.168.0.4:2881;192.168.0.1:2881;192.168.0.3:2881
listen port: 2883
...
admin      38206       1  2 15:11 ?        00:00:00 bin/obproxy -r 192.168.0.4:2881;192.168.0.1:2881;192.168.0.3:2881 -p 2883 -o enable_strict_kernel_release=false,enable_cluster_checkout=false,enable_metadb_used=false -c obdemo
...
* 登录 OBProxy 修改密码
  ...
  1. 登录 OBProxy
     ...
     ```bash
     mysql -h 192.168.0.2 -u root@proxysys -P 2883 -p
  ...
  4. 查看是否部署成功
     ...
     ```bash
     mysql -h192.168.0.2 -uroot@sys#obdemo -P2883 -p4S9wDbSr -c -A oceanbase

     输出：
     [admin@obce00 obproxy-3.1.0]$ mysql -h192.168.0.2 -uroot@sys#obdemo -P2883 -p4S9wDbSr -c -A oceanbase
     Welcome to the MariaDB monitor.  Commands end with ; or \g.
     ...
     +------+--------+------+---------------------+-----------+-------------+-------------------+-------------------+-------+-------+
     |    5 | sys    | root | 192.168.0.2:41524 | oceanbase |           0 |                 1 | MCS_ACTIVE_READER | 38206 | 38206 |
     +------+--------+------+---------------------+-----------+-------------+-------------------+-------------------+-------+-------+
     ...
     +------------+---------+--------+---------------------+-----------+---------+------+--------+-----------------------+---------------+------+--------------+
     | 3222013775 | root    | sys    | 192.168.0.2:57436 | oceanbase | Query   |    0 | ACTIVE | show full processlist | 192.168.0.3 | 2881 |            4 |
     | 3221751633 | proxyro | sys    | 192.168.0.2:49344 | oceanbase | Sleep   |    2 | SLEEP  | NULL                  | 192.168.0.1 | 2881 |            3 |
     +------------+---------+--------+---------------------+-----------+---------+------+--------+-----------------------+---------------+------+--------------+
