...
要使用`dooba`，您必须 MySQL 客户端。建议安装版本 5.5/5.6/5.7 的 MySQL 客户端，因为早期的 OBPROXY （版本低于 2.0.0）可能不完全兼容 MySQL 8.0 的连接协议。
...
* `IOPS`：磁盘 IO 的实时监控。`SESS` 是会话数供参考，量大的时候可能会不准。`IOR` 是读 IOPS，`IOR-SZ` 是读 IO 吞吐量。当第一次读入数据或者数据不在内存中时就会有读物理 IO。读物理 IO 如果非常高，会导致 SQL 的平均延时水平很高，说明内存不足，对 IO 依赖性变大。很可能导致 IO 触达瓶颈。`IOW`是写 IOPS，`IOW-SZ` 是写吞吐量。平时很少有写 IO，因为 OceanBase 的写都在内存里。只有转储和合并的时候，能看到短暂的密集的写 IO。
