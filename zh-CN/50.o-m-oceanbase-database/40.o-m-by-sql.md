...
* 查看索引是否生效
  ...
  ```
  <!-- 这个是什么呢，下面两条都有说明，这个没有 -->

  * sys 租户下查索引是否生效，状态是 `VALID` 表示正常
    ...
    CDB_INDEXES 视图的详细介绍可参见官网《OceanBase 数据库》文档 [参考指南/系统视图/SYS 租户系统视图/字典视图/oceanbase.CDB_INDEXES](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000219602)。
...
* 查看某租户慢事务信息
  ...
  您需将 `${租户ID}` 替换为待查看租户的租户 ID（可通过 `select * from oceanbase.DBA_OB_TENANTS;` 命令获取）。GV\$OB_SQL_AUDIT 视图的详细介绍可参见官网《OceanBase 数据库》文档 [参考指南/性能调优/SQL 调优指南/SQL 优化/SQL 执行性能监控/GV\$OB_SQL_AUDIT](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000220956)。
  <!-- 需要在给出的语句中透出时间么 -->
...
该问题根据出现的场景不同可分为如下三种处理方法，可根据实际场景选择合适的处理方法。
...
3. 如果 GV$OB_SQL_AUDIT 视图中的信息被淘汰、数据库系统日志被刷掉或者系统日志等级太低，可以按以下方式设置，等待 SQL 可执行复现问题时再按上述流程获取日志。
   1. 设置日志等级，默认是 `WDIAG`
      <!-- 默认是 WDIAG，我们为啥还要再设置为 WDIAG 呀 -->

      ```sql
      alter system set syslog_level='WDIAG';
      ```

   2. 设置日志保留个数，设置时需要注意磁盘空间是否能支撑设置的日志数量

      ```sql
      alter system set max_syslog_file_count='10';
      ```
<!-- 也就是说出现这几种情况的话，我们其实无法获取日志信息了，只能等下次复现，对不 -->
#### 场景二：SQL 可重复执行
...
若以上两种方式无法解决问题，您可按以下步骤尝试自查分析。

1. 通过查看 GV$OB_SQL_AUDIT 视图确认影响执行耗时的等待事件。
   <!-- 操作命令 -->

2. 获取 SQL 执行计划 `EXPLAIN EXTENDED`，查看执行计划并分析，具体步骤可参见官网《OceanBase 数据库》文档 [参考指南/性能调优/SQL 调优指南/SQL 优化/SQL 执行性能监控/SQL 性能分析示例/查看执行计划形状并做分析](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000221512)。
   <!-- 这个和上一步之间的联系是什么 -->

3. 获取 sql plan monitor 信息，全过程可以参看如下操作。
   <!-- 是需要按照下述步骤进行操作么，下述步骤和这里的三步的对应关系是怎么样的，明确对应一下 -->
       ```sql
...
<!-- 4.0 这句话有问题，上文说的是可以自查，看最后一步，其实是提供信息找 OB 技术支持 -->
以上的方法适应用于 DML 语句的排查思路，DDL 语句需要按以下方法排查，因为 DDL 卡住场景比较复杂，不适用于用户自查，可以通过提供排查结果，反馈给 OceanBase 技术支持团队协助。
<!-- 这里的以上方法包含哪些，obdiag 也算么，
obdiag 只适用于 执行 SQL 超时或者耗时长 这个场景，其他场景不适用么 -->
...
2. 根据输出的 `tenant_id` 和 `task_id` 等信息，查询 DDL 任务的状态切换历史。
   ...
   如果输出中 `new_state` 的值（对应 ObDDLTaskStatus）是 `WAIT_TRANS_END`，大概率是在等活跃事务结束。
   <!-- 这里说的输出是指下面这几条命令的输出么，具体是哪一条 -->
   ```sql
   ...
   ```
   <!-- 这里需要手动替换 $OBJECT_ID 和 $TARGET_OBJECT_ID 么，怎么获取这两个值-->
...
3. 如果通过 `__all_virtual_ddl_task_status` 表无法获取到 DDL 信息，可能是 Root Service 调度 DDL 线程卡住。
   ...
   如果日志返回信息是表锁相关的 `6005` 或者 `4023` 错误码，可以确认下是否由 `drop/truncate` 操作锁冲突导致，或者将结果提供给 OceanBase 技术支持团队进行分析。
   ...
   如果日志返回信息是 RPC 相关的 `4012` 或者 `4121` 错误码，按以下方式查看。
    ...
    如果返回 DDL 耗时较多，且根据 trace_id 无法在 `__all_virtual_ddl_task_status` 查询到，大概率是业务压力太大导致，可以尝试降低压力恢复；如果返回 DDL 耗时较少且稳定，建议按当前排查过程收集的信息，联系 OceanBase 技术支持同学协助分析。

### 如何排查数据库初始化失败问题

OceanBase 数据库初次部署启动时会有一个 Bootstrap 初始化数据库操作，此步骤只执行一次。如果部署时对资源参数设置不合理或者服务器性能较差等情况导致执行超时（10 分钟），会造成初始化失败，只能重新安装。如果对失败原因不明确，会导致重复安装试错，从而影响使用体验，因此这里将介绍数据库初始化失败的定位排查方式。
<!-- 导致超时是最终结果，还是仅是一个场景 -->
**问题现象**：Bootstrap 初始化阶段失败。

**问题排查**：

无论使用哪种工具部署 OceanBase 数据库，当出现 Bootstrap 初始化阶段失败的情况时，只需要查看第一个 observer.log 日志即可。初始化阶段 observer 进程可能不会退出的，会打印大量 observer 进程日志信息，导致日志被覆盖，干扰问题排查，因此我们需要了解数据库启动时的关键字信息进行定位。
<!-- 第一个 observer.log 日志 是指什么 -->
...
* OBD 升级 OceanBase 数据库失败有如下多个阶段，而日志生成在执行 obd 升级命令的当前目录下。
  ...
  * `Exec upgrade_health_checker.py x`：基本很少遇到此阶段的升级失败，可查看对应日志（`upgrade_cluster_health_checker.log`）分析失败原因。
     <!-- 为啥这两个就不加 失败 了，是前两个有什么特殊的地方么 -->
...
>  
> * 如果自查无法确认问题原因，建议收集以上相关日志信息，联系 OceanBase 技术支持人员协助排查。
...
* OCP 升级 OceanBase 数据库和 OBD 升级数据库逻辑类似，白屏上会实时输出升级过程的日志，比较直观，不过遇到数据库升级脚本失败时，也需要查看 OceanBase 数据库的升级日志，但是和 OBD 升级时日志存放路径和名称不一样。
  ...
  * 升级失败步骤禁止直接跳过或者通过设置为成功的方式绕过。部分升级步骤会有系统参数相关的修改操作，盲目跳过任务，可能导致集群不可用，建议收集以上日志相关信息，联系 OceanBase 技术支持人员协助排查。
...
4. 提供 core 文件、堆栈信息、故障节点事发前后的 observer.log 日志发送到官网 [问答区](https://ask.oceanbase.com/) 或者 OceanBase 社区支持团队，进行分析处理。
