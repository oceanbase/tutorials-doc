...
<!-- CDC （Change Data Capture，即变更数据捕获）能够帮助您监测并捕获数据库的变动。CDC 提供的数据可以做很多事情，比如：做历史库、做近实时缓存、提供给消息队列（MQ），用户消费 MQ 做分析和审计等。--感觉这句很突兀哎 -->

![](70.migration-and-synchronization-through-other-tools-01)
...
![](70.migration-and-synchronization-through-other-tools-02)
...
![](70.migration-and-synchronization-through-other-tools-03)
...
![](70.migration-and-synchronization-through-other-tools-04)
...
![](70.migration-and-synchronization-through-other-tools-05)
...
![](70.migration-and-synchronization-through-other-tools-06)
...
3. 切分多个 Task 之后，DataX Job 会调用 Scheduler 模块，根据配置的并发数据量，将拆分成的 Task 重新组合，组装成 TaskGroup（任务组）。每一个 TaskGroup 负责以一定的并发运行完毕分配好的所有 Task，默认单个任务组的并发数量为 5。
   <!-- 没有看懂  每一个 TaskGroup 负责以一定的并发运行完毕分配好的所有 Task 这句话-->
...
* 在线安装
  ...
  一般情况下不需要安装所有的连接器插件，可以通过配置来指定需要的插件，配置文件地址 `config/plugin_config`。例如，只需要 connector-console 插件，那么可以修改 `plugin.properties` 为如下内容：
   <!-- plugin.properties 是啥,是配置文件里的一部分吗 -->
  ```shell
