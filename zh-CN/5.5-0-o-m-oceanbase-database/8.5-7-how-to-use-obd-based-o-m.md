...
此时需要使用命令 `obd cluster edit-config` 编辑当前运行的集群的配置文件，在 `servers` 下加入新的节点 IP 。新 IP 紧跟在旧的 IP 之后（换行）。

然后针对旧集群运行 `obd cluster start` 命令。这个会启动集群中所有节点。如果节点的进程已经启动，就会跳过。所以，只会启动新增节点的进程。
...
以上命令立即返回，然后查看节点状态可以看到新节点的状态是 `active` 。
