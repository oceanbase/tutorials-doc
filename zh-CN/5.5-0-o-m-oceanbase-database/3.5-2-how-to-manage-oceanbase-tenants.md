...
![资源分配](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0582520461/p375279.jpeg)如上图：
...
集群 sys 租户一样可以设置变量值（sys 租户变量通常不用调整，必须很谨慎），不同租户的变量设置彼此独立，互不影响。

大部分变量的设置是立即生效，生效范围根据设置方法不同可分为实例全局层面生效和会话层面生效。极少数变量（类似 MySQL 的初始化变量）不能在租户里后期修改，只是在 sys 租户里新建业务租户的时候设置。当然也可以后期在 sys 租户后期修改，只是在修改时必须充分评估影响。
...
新建租户常用租户变量修改。可修改语句超时时间，避免复杂的语句查询时超时报错。 如下面是语句超时报错示例：
