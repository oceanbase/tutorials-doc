...
```bash
[root@obce01 ~]# clockdiff 192.168.0.1
...
host=192.168.0.1 rtt=428(314)ms/0ms delta=0ms/0ms Thu Sep 30 21:33:16 2021
[root@obce01 ~]# clockdiff 192.168.0.3
...
host=192.168.0.3 rtt=426(314)ms/0ms delta=0ms/0ms Thu Sep 30 21:33:22 2021

[root@obce02 ~]# clockdiff 192.168.0.2
.
host=192.168.0.2 rtt=750(187)ms/0ms delta=0ms/0ms Thu Sep 30 21:34:10 2021
[root@obce02 ~]# clockdiff 192.168.0.4
...
host=192.168.0.4 rtt=423(315)ms/0ms delta=-1ms/-1ms Thu Sep 30 21:34:14 2021
```

在某些客户机房环境下，命令 `clockdiff` 可能会报错（原因不明）。此时可使用 `ping` 命令，查看返回的信息判断时间误差。

```bash
ping -T tsandaddr 192.168.0.1 -c 3

输出：
[root@obce01 ~]# ping -T tsandaddr 192.168.0.1 -c 3
PING 192.168.0.1 (192.168.0.1) 56(124) bytes of data.
64 bytes from 192.168.0.1: icmp_seq=1 ttl=64 time=26.3 ms
TS:     192.168.0.4   48963954 absolute
        192.168.0.1   13
        192.168.0.1   0
        192.168.0.4   13

64 bytes from 192.168.0.1: icmp_seq=2 ttl=64 time=0.333 ms
TS:     192.168.0.4   48964963 absolute
        192.168.0.1   1
        192.168.0.1   0
        192.168.0.4   0

64 bytes from 192.168.0.1: icmp_seq=3 ttl=64 time=0.480 ms
TS:     192.168.0.4   48966011 absolute
        192.168.0.1   0
        192.168.0.1   0
        192.168.0.4   0
