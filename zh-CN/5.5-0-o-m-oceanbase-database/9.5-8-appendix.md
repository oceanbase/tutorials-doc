...
本文介绍 OBAgent 常用指标的查询表达式。有关 OBAgent 的具体介绍，请参考 [OBAgent 文档](../2.chapter-2-how-to-deploy-oceanbase-community-edition/10.2-9-how-to-deploy-obagent.md)。
...
| 平均每次读取耗时                    | `1000000 * (avg(rate(node_disk_read_time_seconds_total{@LABELS}[@INTERVAL])) by (@GBLABELS)) / (avg(rate(node_disk_reads_completed_total{@LABELS}[@INTERVAL])) by (@GBLABELS))`   | us       |
...
| 平均每次写入耗时                    | `1000000 * (avg(rate(node_disk_write_time_seconds_total{@LABELS}[@INTERVAL])) by (@GBLABELS)) / (avg(rate(node_disk_writes_completed_total{@LABELS}[@INTERVAL])) by (@GBLABELS))`  | us       |
