...
复制表的结构用 `like`，包括主键、唯一键、索引名称都会复制。在 MySQL 语法里，主键名、唯一约束和索引名在一个表内不能重复，但是不同表之间可以重复。

```bash
create table t1 like ware;
```

复制表的结构和数据用 `create table ... as select`。不过要注意，这个复制的是表的基本数据类型，对于主键、唯一约束和索引信息等不会复制。
